...
import com.E22214005_SHY.StudentSystem.dao.ManageHelper;
import com.E22214005_SHY.StudentSystem.model.ScoreModel;
import com.E22214005_SHY.StudentSystem.util.CreateSql;
import com.E22214005_SHY.StudentSystem.util.Tools;
import com.E22214005_SHY.StudentSystem.util.WindowUtil;
import com.E22214005_SHY.StudentSystem.viewer.StudentScoreViewer;
	...
	public QueryStudentScoreFrame(JFrame owner, String title, boolean modal) {
		...
		details_Button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				// 获取学生ID
				String studentId = (String) jt.getValueAt(rowNum, 0); // 假设学生ID在第一列
				// 数据型显示
				QueryScoreFrame frame = new QueryScoreFrame(jd, "学生成绩", true, scoreModel, rowNum);
				// 打开 StudentGradeViewer 窗口，成绩曲线显示
				StudentScoreViewer.displayStudentGrades(studentId);
			}
		});
